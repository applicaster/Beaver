# Beaver — Wire Protocol Contract

This document captures the WebSocket protocol that Beaver must speak
...
where v6 is preferred.)** Beaver will accept both unless the SDK side
forbids it.
...
- `id` — UUID generated by Beaver for this connection.
...
**Future improvement** (would let Beaver delete its registry):
...
2. **Namespace stability.** Are there ever namespaces other than the
   three above? If the SDK supports custom namespaces, Beaver should
...
| `timing.endTime`    | number, ms since epoch          | optional     | used to derive `duration` when `timing.duration` is absent |
| `timing.duration`   | number, ms                      | optional     | preferred over `endTime − startTime` when present |
| `timestamp`         | number, ms since epoch          | optional     | fallback for `timing.startTime` |
| `status`            | number                          | optional     | absent means a transport failure (see `error`) |
