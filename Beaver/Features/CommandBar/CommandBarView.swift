...
private struct CommandHelpPopover: View {
    ...
    private var empty: some View {
        VStack(alignment: .center, spacing: 8) {
            ...
            Text("No commands yet")
                .font(.caption)
            Text("Beaver asked the connected device for its command list. If this stays empty, click ⟳ to retry.")
                .font(.caption2)
                .foregroundStyle(.secondary)
    ...
    /// Beaver" section for commands the registry doesn't cover.
    private var groups: [(title: String, hints: [CommandHint])] {
        ...
        if !unknown.isEmpty {
            result.append(("Unknown to Beaver", unknown.sorted { $0.name < $1.name }))
        }
        return result
    }
}
